...
void process_ip_packet(struct sr_instance* sr,
    ...
    if(ip_hdr->ip_ttl == 0) {
        send_ICMP_message(sr, packet, len, (uint8_t)11 , (uint8_t)0);
        return;
    }
    ...
    if(sr_get_interface(  sr,interface) == NULL){
        ...
        if(!routing_entry) {  
            send_ICMP_message(sr, packet, len, (uint8_t)3 , (uint8_t)0);
            return;
        }
        ...
        if(ip_hdr->ip_p == ip_protocol_icmp){
            ...
            if (icmp_hdr->icmp_code == (uint8_t) 0 ) {
                /* send echo reply  */
                send_ICMP_message(sr, packet, len, (uint8_t)0, (uint8_t) NULL);
            }
        }
        else if (ip_hdr->ip_p == ip_protocol_udp || ip_hdr->ip_p == ip_protocol_tcp) {
            /* Send port Unreachable message */
            send_ICMP_message(sr, packet, len, (uint8_t)3, (uint8_t)3);
        }
    }
}
...
void process_arp_packet(struct sr_instance* sr,
        ...
        if(sr_get_interface( sr,arp_hdr->ar_tip ) == NULL){
        ...
        if(ntohs(arp_hdr->ar_op) == arp_op_request){
            ...
            uint8_t* new_rep = (uint8_t) malloc(sizeof(sr_ethernet_hdr_t) + sizeof(sr_arp_hdr_t));
...
void send_ICMP_message(struct sr_instance* sr, uint8_t* packet, unsigned int len, uint8_t type, uint8_t code){
    ...
    if(type == 0 & code == NULL){
        ...
        memcpy(eth_hdr->ether_shost, 0,  sizeof(uint8_t)*ETHER_ADDR_LEN);
        /* IP */
